			...
			for(auto& VitalityItem : RaceData.Items)
			{
				...
				DON(Num,
					...
					if(UVoxelModuleStatics::VoxelAgentTraceSingle(Index, VitalityData.Radius, VitalityData.HalfHeight, {}, HitResult, true, 10, false, false))
					{
						...
						SaveData.InitData(WorldData.RandomStream);
						UAbilityModuleStatics::SpawnAbilityActor(&SaveData, UVoxelModuleStatics::GetChunkByLocation(SaveData.SpawnTransform.GetLocation()));
					}
				)
			}
		}
